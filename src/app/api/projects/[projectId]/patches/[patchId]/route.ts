...
import { getPatch, listPatchApplications, listPatchFacts } from "@/server/db/canon-patch";
...
export async function GET(_request: Request, context: RouteContext) {
  ...
  try {
    ...
    return patch ? NextResponse.json({ data: { patch, applications: listPatchApplications(projectId, patchId), facts: listPatchFacts(projectId, patchId) } }) : notFoundResponse("Patch not found");
  } catch (error) {
