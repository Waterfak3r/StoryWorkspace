...
- Windows 下若标准启动因 npm shim 报 `%1 is not a valid Win32 application`，保留 pane 并用 `herdr pane run <paneId> "claude.cmd --permission-mode auto"` 恢复；随后必须确认 Herdr 已识别 Claude、界面显示 `auto mode on`，再命名 agent 并派发任务。
