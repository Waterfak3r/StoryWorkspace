...
7. 架构、代理依赖、冲突由主代理处理。对抗审查单向：agy 审界面并对 Grok 后端给建议；Grok 不审 agy 的后端。
8. 切片后主代理审报告与测试输出。主代理不跑测试；合同验证缺失或相关测试失败才退回补跑。界面切片还要过 agy 观感审查。禁止为小问题整仓重跑。
9. 未满足验收不得宣称完成。不要在很长、刚压缩或余额不足的会话上开 `/goal`。用户当场纠正产品后写入 `mvp.md`。
