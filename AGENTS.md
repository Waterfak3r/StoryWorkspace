...
- Herdr 协作者只有 Antigravity（Gemini 3.7 Flash，kind `agy`）。能力弱于主代理与 4.6 子代理，只做边界清晰的机械性、批量或局部实现。不使用 Claude。派发必须写明路径、约束和验收条件；主代理检查输出后才可视为完成。启动：`herdr agent start <name> --kind agy --pane <paneId>`，cwd 保持仓库根目录。
- Antigravity 不自行改变产品范围或核心架构；发现问题时提交证据与建议。小改主代理可直接做。
...
4. 派 4.6 / medium 子代理实现。文案、批量、机械改动可走 Antigravity 或由主代理直接改。
5. 先打通文件真相上的前后端闭环，再扩展同一能力。
6. 架构、代理依赖、冲突由主代理处理。
7. 切片后主代理审报告与测试输出。主代理不跑测试；合同验证缺失或相关测试失败才退回补跑。
8. 未满足验收不得宣称完成。
