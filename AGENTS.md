...
- 歧义交回给我决策或问用户。4.5 + medium effort 足够执行；主代理不得把未想清的任务丢给子代理去“想”。
...
3. 我明确用户故事、当前切片、接口、文件归属与验收条件。
4. 实现切片用 `spawn_subagent` 分发给 Grok 子代理（`grok-4.5`）。文案、批量重命名、机械修补、i18n 填表等走 Herdr Claude。
...
- 显著改变范围、数据模型、持久化或用户体验的歧义，由我决策或询问用户。
