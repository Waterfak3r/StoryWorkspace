...
- 当前 Phase 0–4 采用方式：在 SQLite/Next 本地单体内落地稳定 revision、deterministic scene analysis、Canon Patch review、document-scoped continuity group、独立 EntityState resolver，以及不调用 Provider 的 deterministic Context Snapshot/Inspector；这不是 PostgreSQL、托管队列、RAG、复杂故事时间线或媒体 Provider 已接入的声明。
