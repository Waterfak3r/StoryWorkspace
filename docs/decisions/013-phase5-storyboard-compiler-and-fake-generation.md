...
4. Phase 5B 引入代码拥有且带版本的 `fake-video` capability profile，以及项目内 approved reference-image metadata。首个本地切片的 reference record 仅代表经作者创建的已批准元数据，不声称已上传或保存图片二进制。Compiler 输入只包含 Context Snapshot、ShotSpec、capability profile 与明确选择的 reference asset IDs；输出 provider-neutral prompt segments、asset inputs、参数、warning 和 omitted context，不负责提交或重试。
...
6. Compiler output 使用 canonical JSON + SHA-256，固定 compiler version 和 capability profile version。相同 Snapshot + ShotSpec + profile + asset versions + 参数产生相同 compiled hash；预览持久化为不可变 compiled request，并通过 Fake Adapter 的 validate/prepare 边界派生完整 `fake://video/generate` prepared request，供 Manifest 精确引用。Phase 5B 不执行 submit。
