...
- 否则创建实体 / 在选定 Volume/Chapter 下创建场（请求体 `volumeId` + `chapterId`；省略时仍回退 `volume-01` / `chapter-01`），并写上 id 引用（`characters[]` / `location`）。已有同 title 场仍原地更新。Confirm 不是整本故事覆盖。
...
错误码沿用 Slice 01。无效 LLM JSON → 502/`AI_INVALID_RESPONSE` 或 400，且不写 scene/entity。

## UI

Story 区增加「Paste & parse」：文本框、Parse、待确认列表、Confirm / Reject。文案走 i18n。用户正文不翻译。Confirm 把**新**场写进当前选中的 Volume/Chapter；解析确认不是整本故事覆盖。已匹配 title 的场仍在原位置更新。
