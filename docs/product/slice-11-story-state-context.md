...
3. `resolveContext`：实体 `state` = `states.default` 叠加上故事顺序里**本场之前**所有补丁。快照增加可检查的 `storyPosition`（前若干事件的 title+summary，截断）。
...
- 场 A 给角色 condition=injured，场 B 的 snapshot.state.condition 为 injured；场 A 仍是 default。
