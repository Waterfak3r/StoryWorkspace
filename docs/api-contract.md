...
Phase 5B compiler preview routes:

- `POST /api/projects/{projectId}/reference-assets` creates approved local reference-image metadata for one same-project active/draft Entity; `GET ...?entityId=` lists strict project/entity-scoped records. This command does not upload or claim an image binary.
- `POST /api/projects/{projectId}/shots/{shotSpecId}/compile` accepts the `fake-video-v1` profile, selected reference metadata IDs, requested duration/aspect parameters, request ID, and actor. It returns 201 for a new immutable compiled request or 200 for request/semantic replay.
- `GET /api/projects/{projectId}/compiled-requests/{compiledRequestId}` returns the immutable provider-neutral request and exact Fake Adapter prepared preview.

Compilation requires an approved Storyboard, non-deleted Scene, non-blocking bound Context Snapshot, and same-project approved reference metadata linked to a Shot subject/location/prop. The Fake profile supports a text prompt, negative prompt, at most two references, 4/6/8 seconds, and 16:9/9:16. Deterministic clipping and duration/aspect fallback are explicit in `warnings` and `omittedContext`. `fake://video/generate` is a local prepared-request boundary only; no submit, network access, credential, billing, Manifest, job, result, or media asset is created in Phase 5B.

Phase 5C generation routes:

- `POST /api/projects/{projectId}/generations` accepts one immutable `compiledRequestId`, request ID, actor, and an optional strict Fake-only behavior. It creates/replays the Manifest and its single Job, then traverses the Fake Adapter submit/status/result boundary.
- `GET /api/projects/{projectId}/generations/{manifestId}` returns the strict Manifest + Job + nullable Result inspector record, including the complete frozen source identity, exact prepared request, normalized parameters/error, attempts, provider job ID, result URI, metadata, and hashes.
- `POST /api/projects/{projectId}/generation-jobs/{jobId}/retry` accepts expected Job version, request ID, and actor. Only a failed retryable Job may re-enter the lifecycle; request replay is idempotent and stale versions conflict.

`success` deterministically yields an immutable `fake://video/results/...` Result. The local `timeout_after_accept_once` fixture persists one accepted Fake provider submission before returning a normalized timeout; retry uses the same provider idempotency key and reads that submission instead of submitting again. The adapter performs no network access, secret lookup, billing, upload, or real media creation. Unknown request/response fields are rejected, and every ID read/write is project scoped.
...
- Phase 0–5C migrations bring the local schema to version 17 from the MVP baseline. New cross-project references are checked in repositories and SQLite project-guard triggers; revisions, evidence, analysis identities, patch payload/provenance, Fact values, EntityState values, Context Snapshot content, ShotSpec content, reference metadata, compiled requests/previews, Generation Manifests, and Results are immutable, with only documented lifecycle/version/latest transitions. Pending Patches are Canon-only review commands with operation-specific target/baseVersion and application-result shape; Patch status/version, Inference lifecycle, and Generation Job transitions are trigger-guarded. v11 enforces Fact/Inference scope shape and same-project resolvable entity references; v12 requires accepted non-null ModelRuns to be succeeded and source-bound; v13 adds document-scoped continuity, state provenance, and atomic `add_state` acceptance; v14 additively stores provider-neutral Context Snapshots whose latest index can change without changing content; v15 binds immutable Storyboards/ShotSpecs to a Snapshot and uses CAS lifecycle transitions; v16 binds approved reference metadata and capability/compiler-versioned Fake Adapter previews; v17 freezes the submitted chain and records exactly-once Fake submissions, jobs, and normalized results. A new revision that removes accepted text evidence creates a reviewable Fact retract suggestion; it never directly mutates Canon.
